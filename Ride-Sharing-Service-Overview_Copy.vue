...
<!-- Right Panel - Available Rides / Passenger Requests -->
<div class="w-full lg:w-2/3">
<div class="bg-white rounded-xl shadow-lg p-6 mb-6">
<div class="flex justify-between items-center mb-6">
<h2 class="text-2xl font-bold">{{ userMode === 'passenger' ? 'Available Rides' : 'Passenger Requests' }}</h2>
<div class="flex items-center">
...
<!-- Ride/Request Cards -->
<div class="space-y-4">
<div v-if="userMode === 'passenger'" v-for="(ride, index) in availableRides" :key="index" class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
...
<button class="bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-md font-medium mt-4 transition-all whitespace-nowrap !rounded-button cursor-pointer">
Book Seat
</button>
</div>
</div>
...
<p class="text-gray-500">{{ userMode === 'passenger' ? 'No rides available for your search criteria.' : 'No passenger requests available.' }}</p>
<p class="text-gray-500">{{ userMode === 'passenger' ? 'Try adjusting your filters or search for a different date.' : 'Check back later for new requests.' }}</p>
...
<a-modal
  ...
  width="800px"
  @ok="confirmBooking"
  :okText="selectedRide?.booked ? 'Already Booked' : 'Book Seat'"
  :okButtonProps="{ disabled: selectedRide?.booked }"
>
  <div class="space-y-6">
    <!-- Map Section -->
    <div class="h-64 bg-gray-100 rounded-lg overflow-hidden relative">
      <img
        src="https://readdy.ai/api/search-image?query=detailed%20city%20map%20showing%20route%20between%20two%20points%20with%20exact%20pickup%20and%20dropoff%20locations%20marked%2C%20modern%20map%20interface%20design%20with%20street%20names%20and%20landmarks%20visible%2C%20clean%20design%20with%20route%20highlighted&width=800&height=256&seq=routemap1&orientation=landscape"
        alt="Route Map"
      ...
      <div>
        ...
        <div class="space-y-3">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-medium">{{ selectedRide?.duration }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600">Price:</span>
            <span class="font-bold text-red-600">${{ selectedRide?.price }}</span>
          </div>
        </div>
      </div>

      <div>
        <h3 class="font-bold text-lg mb-4">Vehicle & Passengers</h3>
        <div class="space-y-4">
          <div class="bg-gray-50 p-3 rounded-lg">
            <p class="font-medium">{{ selectedRide?.vehicle }}</p>
            <p class="text-sm text-gray-500">{{ selectedRide?.availableSeats }} seats available</p>
          </div>
          <div class="space-y-2">
            <p class="font-medium mb-2">Other Passengers:</p>
            <div v-for="passenger in currentPassengers" :key="passenger.id" class="flex items-center justify-between bg-gray-50 p-2 rounded-lg">
              <div class="flex items-center">
                <img :src="passenger.avatar" alt="passenger" class="w-8 h-8 rounded-full mr-2" />
                <div>
                  <p class="font-medium">{{ passenger.name }}</p>
                  <p class="text-sm text-gray-500">{{ passenger.email }}</p>
                </div>
              </div>
              <safety-certificate-outlined class="text-green-500" />
            </div>
          </div>
        </div>
      </div>
...
const bookedRides = ref(new Set());

// Current passengers data
const currentPassengers = ref([
  {
    id: 1,
    name: 'Alice Johnson',
    email: 'aj123@cornell.edu',
    avatar: 'https://readdy.ai/api/search-image?query=professional%20headshot%20of%20a%20young%20woman%20smiling%2C%20natural%20lighting%2C%20simple%20background&width=100&height=100&seq=passenger1&orientation=squarish'
  },
  {
    id: 2,
    name: 'Bob Smith',
    email: 'bs234@cornell.edu',
    avatar: 'https://readdy.ai/api/search-image?query=professional%20headshot%20of%20a%20young%20man%20smiling%2C%20natural%20lighting%2C%20simple%20background&width=100&height=100&seq=passenger2&orientation=squarish'
  }
]);

// Passenger requests data
const passengerRequests = ref([
  {
    passengerName: 'Emma Wilson',
    passengerImage: 'https://readdy.ai/api/search-image?query=professional%20headshot%20of%20a%20young%20woman%20with%20brown%20hair%20smiling%2C%20natural%20lighting%2C%20simple%20background&width=200&height=200&seq=passenger3&orientation=squarish',
    cornellEmail: 'ew345@cornell.edu',
    rating: 4.9,
    origin: 'Cornell Tech Campus',
    destination: 'Penn Station',
    pickupTime: '9:00 AM, Apr 3',
    seats: 1,
    note: 'Small luggage',
    offeredPrice: 30
  },
  {
    ...
    passengerImage: 'https://readdy.ai/api/search-image?query=professional%20headshot%20of%20a%20young%20man%20with%20glasses%20smiling%2C%20natural%20lighting%2C%20simple%20background&width=200&height=200&seq=passenger4&orientation=squarish',
    cornellEmail: 'mb456@cornell.edu',
    rating: 4.7,
    origin: 'Collegetown',
    destination: 'JFK Airport',
    pickupTime: '10:30 AM, Apr 3',
    seats: 2,
    note: 'Two large suitcases',
    offeredPrice: 45
  }
]);

// Functions for booking modal
...
const confirmBooking = () => {
  if (selectedRide.value) {
    bookedRides.value.add(selectedRide.value);
...
const isRideBooked = (ride) => {
  return bookedRides.value.has(ride);
};
